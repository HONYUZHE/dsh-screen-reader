...
import { BridgeError, dump, errorText } from './bridge.js'
...
export class ScreenReader {
  ...
  async read(options = {}) {
    ...
    let attempts
    try {
      ({ raw, attempts } = await this.fetchStable({ signal, timeoutMs }))
    } catch (error) {
      // 工具层只把 message 交给模型，hint 会被丢掉 —— 而「该去哪开开关」恰恰是
      // 用户必须知道的信息。所以在这里把 hint 并进 message，让它一路传到模型面前。
      throw error?.hint ? new BridgeError(errorText(error), error.code ?? 'BRIDGE_ERROR') : error
    }
