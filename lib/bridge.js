...
export function classifyResult(result) {
  ...
  // 必须**锚定行首**——屏幕上的对话正文里完全可能出现这句话（实测踩过），
  // 锚不住就会把一次成功的 dump 误判成拒绝。
  if (/^(?:\[ERR\]\s*)?你拒绝了这次(?:屏幕读取|截屏|屏幕|读取)\s*$/.test(result.trim())) {
    return { code: 'SCREEN_DENIED', hint: '' }
  }
  const marker = /^\[([A-Z_]{3,40})\]/.exec(result)
  if (marker) return { code: marker[1], hint: '' }
  const meta = /^(DISABLED|NO_PERMISSION|UNAUTHORIZED)\b/.exec(result)
