...
/**
 * 把一个错误渲染成给模型看的完整文本：桥的原话 + 我们知道的下一步建议。
 *
 * **一定要用它**，别只抛 `error.message`：`hint` 承载的是「该去哪开开关」这类
 * 用户必须知道的信息，只抛 message 会让 {@link hintFor} 变成死代码 ——
 * 模型看到一个「你拒绝了这次屏幕读取」却不知道该怎么办，就会去瞎重试。
 * @param {unknown} error 任意错误。
 * @returns {string} 可直接放进工具结果/消息的文本。
 */
export function errorText(error) {
  const message = error?.message ?? String(error ?? '')
  const hint = error?.hint
  if (typeof hint === 'string' && hint && !message.includes(hint)) return `${message}\n→ ${hint}`
  return message
}
